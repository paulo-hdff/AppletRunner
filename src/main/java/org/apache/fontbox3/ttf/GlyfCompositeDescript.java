...
public class GlyfCompositeDescript extends GlyfDescript
{
     ...
     * Log instance.
     */
    private static final Logger LOG = Logger.getLogger(GlyfCompositeDescript.class.getName());;
    ...
    public void resolve()
    {
        ...
        if (beingResolved)
        {
            LOG.log(Level.SEVERE,"Circular reference in GlyfCompositeDesc");
            return;
        }
        beingResolved = true;
    ...
    public int getPointCount()
    {
        if (!resolved)
        {
            LOG.log(Level.SEVERE,"getPointCount called on unresolved GlyfCompositeDescript");
        }
        if (pointCount < 0)
        {
            ...
            if (gd == null)
            {
                LOG.log(Level.SEVERE,"GlyphDescription for index " + c.getGlyphIndex() + " is null, returning 0");
                pointCount = 0;
            }
            else
            {
    ...
    public int getContourCount()
    {
        if (!resolved)
        {
            LOG.log(Level.SEVERE,"getContourCount called on unresolved GlyfCompositeDescript");
        }
        if (contourCount < 0)
        {
            GlyfCompositeComp c = components.get(components.size() - 1);
            ...
            if (gd == null)
            {
                LOG.log(Level.SEVERE,"missing glyph description for index " + c.getGlyphIndex());
                contourCount = 0;
            }
            else
            {
