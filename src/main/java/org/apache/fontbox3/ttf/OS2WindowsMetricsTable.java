...
public class OS2WindowsMetricsTable extends TTFTable
{
     ...
     * Log instance.
     */
    private static final Logger LOG = Logger.getLogger(OS2WindowsMetricsTable.class.getName());;
    ...
    void read(TrueTypeFont ttf, TTFDataStream data) throws IOException
    {
        ...
        {
            LOG.log(Level.FINER,"EOF, probably some legacy TrueType font");
            initialized = true;
            return;
        }
        if (version >= 1)
        {
            ...
            {
                version = 0;
                LOG.log(Level.WARNING,"Could not read all expected parts of version >= 1, setting version to 0", ex);
                initialized = true;
                return;
            }
        }
        if (version >= 2)
        {
            ...
            {
                version = 1;
                LOG.log(Level.WARNING,"Could not read all expected parts of version >= 2, setting version to 1", ex);
                initialized = true;
                return;
            }
        }
