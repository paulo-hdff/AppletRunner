...
{
    private static final Logger LOG = Logger.getLogger(CmapSubtable.class.getName());;
    ...
    void processSubtype8(TTFDataStream data, int numGlyphs) throws IOException
    {
        ...
        if (numGlyphs == 0)
        {
            LOG.log(Level.WARNING,"subtable has no glyphs");
            return;
        }
        // -- Read all sub header
    ...
    void processSubtype14(TTFDataStream data, int numGlyphs) throws IOException
    {
        ...
        LOG.log(Level.WARNING,"Format 14 cmap table is not supported and will be ignored");
    }
    ...
    void processSubtype2(TTFDataStream data, int numGlyphs) throws IOException
    {
        ...
        for (int i = 0; i <= maxSubHeaderIndex; ++i)
        {
            ...
            for (int j = 0; j < entryCount; ++j)
            {
                ...
                if (p >= numGlyphs)
                {
                    LOG.log(Level.WARNING,"glyphId " + p + " for charcode " + charCode + " ignored, numGlyphs is " + numGlyphs);
                    continue;
                }
