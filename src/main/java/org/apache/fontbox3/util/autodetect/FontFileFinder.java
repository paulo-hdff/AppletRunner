...
package org.apache.fontbox3.util.autodetect;
...
{
    private static final Logger LOG = Logger.getLogger(FontFileFinder.class.getName());;
        ...
        for (File file : filelist)
        {
            if (file.isDirectory())
            {
                ...
                if (file.isHidden())
                {
                    ...
                    {
                        LOG.log(Level.FINER,"skip hidden directory " + file);
                    }
                    continue;
                }
                walk(file, results);
            ...
            else
            {
                if (LOG.isLoggable(Level.FINER))
                {
                    LOG.log(Level.FINER,"checkFontfile check " + file);
                }
                if (checkFontfile(file))
                {
                    if (LOG.isLoggable(Level.FINER))
                    {
                        LOG.log(Level.FINER,"checkFontfile found " + file);
                    }
