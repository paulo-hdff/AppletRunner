...
{
    private static final Logger LOG = Logger.getLogger(Type1CharStringParser.class.getName());
    ...
    private void processCallSubr(List<byte[]> subrs, List<Object> sequence) throws IOException
    {
        ...
        {
            LOG.log(Level.WARNING,"Parameter " + obj + " for CALLSUBR is ignored, integer expected in glyph '"
        ...
        {
            LOG.log(Level.WARNING,"CALLSUBR is ignored, operand: " + operand + ", subrs.size(): " + subrs.size()
    ...
    private void processCallOtherSubr(DataInput input, List<Object> sequence) throws IOException
    {
        ...
        if (!results.isEmpty())
        {
            LOG.log(Level.WARNING,"Value left on the PostScript stack in glyph " + currentGlyph + " of font "
                    + fontName);
        }
    }
