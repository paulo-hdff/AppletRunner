...
package org.apache.pdfbox3.pdmodel.encryption;
...
public abstract class SecurityHandler<T_POLICY extends ProtectionPolicy>
{
     ...
     * @param documentIDArray  document id which is returned via {@link org.apache.pdfbox3.cos.COSDocument#getDocumentID()}
    ...
    private void encryptDataAES256(InputStream data, OutputStream output, boolean decrypt) throws IOException
    {
        ...
        catch (IOException exception)
        {
            ...
            LOG.log(Level.FINER,"A GeneralSecurityException occurred when decrypting some stream data", exception);
        }
    }
    ...
    public void decryptStream(COSStream stream, long objNum, long genNum) throws IOException
    {
        ...
        if (COSName.METADATA.equals(type))
        {
            ...
            try (InputStream is = stream.createRawInputStream())
            {
                ...
                {
                    LOG.log(Level.FINER,"Tried reading " + buf.length + " bytes but only " + isResult + " bytes read");
                }
            }
            if (Arrays.equals(buf, "<?xpacket ".getBytes(StandardCharsets.ISO_8859_1)))
            {
                LOG.log(Level.WARNING,"Metadata is not encrypted, but was expected to be");
                LOG.log(Level.WARNING,"Read PDF specification about EncryptMetadata (default value: true)");
                return;
            }
        }
        ...
        {
            LOG.log(Level.SEVERE,ex.getClass().getSimpleName() + " thrown when decrypting object " +
                    objNum + " " + genNum + " obj");
            throw ex;
        }
    }
    ...
    private void decryptString(COSString string, long objNum, long genNum)
    {
        ...
        {
            LOG.log(Level.SEVERE,"Failed to decrypt COSString of length " + string.getBytes().length + 
