...
{
    private static final Logger LOG = Logger.getLogger(PDCIDFontType0.class.getName());
    ...
    public PDCIDFontType0(COSDictionary fontDictionary, PDType0Font parent) throws IOException
    {
            ...
            if (ff3Stream != null)
            {
                try (RandomAccessRead randomAccessRead = ff3Stream.getCOSObject().createView())
                {
                    if (randomAccessRead.length() > 0 && randomAccessRead.peek() == '%')
                    {
                        ...
                        LOG.log(Level.WARNING,"Found PFB but expected embedded CFF font " + fd.getFontName());
                        fontIsDamaged = true;
                    }
                    else
                    {
                ...
                {
                    LOG.log(Level.SEVERE,"Can't read the embedded CFF font " + fd.getFontName(), e);
                    fontIsDamaged = true;
                }
            }
        }
        ...
        else
        {
            ...
            {
                LOG.log(Level.WARNING,"Using fallback " + font.getName() + " for CID-keyed font " +
                         getBaseFont());
            }
            isEmbedded = false;
    ...
    private BoundingBox generateBoundingBox()
    {
        ...
        {
            LOG.log(Level.FINER,"Couldn't get font bounding box - returning default value", e);
