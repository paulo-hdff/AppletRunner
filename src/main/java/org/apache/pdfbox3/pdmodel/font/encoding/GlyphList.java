 ...
 */
package org.apache.pdfbox3.pdmodel.font.encoding;
...
{
    private static final Logger LOG = Logger.getLogger(GlyphList.class.getName());
        ...
        try (BufferedReader in = new BufferedReader(new InputStreamReader(input, StandardCharsets.ISO_8859_1)))
        {
            while (in.ready())
            {
                ...
                if (line != null && !line.startsWith("#"))
                {
                    ...
                    {
                        LOG.log(Level.WARNING,"duplicate value for " + name + " -> " + parts[1] + " " +
            ...
            else if (name.startsWith("uni") && name.length() == 7)
            {
                ...
                try
                {
                    for (int chPos = 3; chPos + 4 <= nameLength; chPos += 4)
                    {
                        ...
                        {
                            LOG.log(Level.WARNING,"Unicode character name with disallowed code area: " + name);
                        }
                        else
                        {
                ...
                {
                    LOG.log(Level.WARNING,"Not a number in Unicode character name: " + name);
                }
            }
