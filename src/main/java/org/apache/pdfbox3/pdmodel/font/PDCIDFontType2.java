...
import org.apache.fontbox3.cmap.CMap;
...
{
    private static final Logger LOG = Logger.getLogger(PDCIDFontType2.class.getName());
    ...
    public PDCIDFontType2(COSDictionary fontDictionary, PDType0Font parent, TrueTypeFont trueTypeFont) throws IOException
    {
        ...
        else
        {
            ...
            if (stream != null)
            {
                ...
                {
                    fontIsDamaged = true;
                    LOG.log(Level.WARNING,"Could not read embedded OTF for font " + getBaseFont(), e);
                }
                if (ttfFont instanceof OpenTypeFont && !((OpenTypeFont) ttfFont).isSupportedOTF())
                {
                    ...
                    ttfFont = null;
                    fontIsDamaged = true;
                    LOG.log(Level.WARNING,"Found an OpenType font using CFF2 outlines which are not supported "
                            + fd.getFontName());
                }
    ...
    private TrueTypeFont findFontOrSubstitute() throws IOException
    {
        ...
        {
            LOG.log(Level.WARNING,"Using fallback font " + ttfFont.getName() +
    ...
    public int codeToGID(int code) throws IOException
    {
        if (!isEmbedded)
        {
            ...
            if (cid2gid != null && !isDamaged && name != null && name.equals(ttf.getName()))
            {
                ...
                LOG.log(Level.WARNING,"Using non-embedded GIDs in font " + getName());
            ...
            else
            {
                ...
                if (unicode == null)
                {
                    if (!noMapping.contains(code))
                    {
                        ...
                        noMapping.add(code);
                        LOG.log(Level.WARNING,"Failed to find a character mapping for " + code + " in " + getName());
                    }
                ...
                {
                    LOG.log(Level.WARNING,"Trying to map multi-byte character using 'cmap', result will be poor");
                }
