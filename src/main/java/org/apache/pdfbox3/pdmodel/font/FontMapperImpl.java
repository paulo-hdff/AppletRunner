...
{
    private static final Logger LOG = Logger.getLogger(FontMapperImpl.class.getName());
    ...
    private FontInfo getFont(FontFormat format, String postScriptName)
    {
        ...
        if (info != null && info.getFormat() == format)
        {
            ...
            {
                LOG.log(Level.FINER,String.format("getFont('%s','%s') returns %s", format, postScriptName, info));
            }
            return info;
        }
        return null;
        ...
        if (cidSystemInfo != null)
        {
                ...
                collection.equals("Adobe-Japan1") || collection.equals("Adobe-Korea1"))
            {
                ...
                if (bestMatch != null)
                {
                    if (LOG.isLoggable(Level.FINER))
                    {
                        LOG.log(Level.FINER,"Best match for '" + baseFont + "': " + bestMatch.info);
                    }
