 ...
 */
package org.apache.pdfbox3.pdmodel.font;
...
import org.apache.fontbox3.FontBoxFont;
import org.apache.fontbox3.cff.CFFCIDFont;
import org.apache.fontbox3.cff.CFFFont;
import org.apache.fontbox3.ttf.NamingTable;
import org.apache.fontbox3.ttf.OS2WindowsMetricsTable;
import org.apache.fontbox3.ttf.OTFParser;
import org.apache.fontbox3.ttf.OpenTypeFont;
import org.apache.fontbox3.ttf.TTFParser;
import org.apache.fontbox3.ttf.TrueTypeCollection;
import org.apache.fontbox3.ttf.TrueTypeFont;
import org.apache.fontbox3.type1.Type1Font;
import org.apache.fontbox3.util.autodetect.FontFileFinder;
import org.apache.pdfbox3.io.RandomAccessReadBufferedFile;
...
{
    private static final Logger LOG = Logger.getLogger(FileSystemFontProvider.class.getName());
    ...
    private static class FSFontInfo extends FontInfo
    {
        ...
        private TrueTypeFont getTrueTypeFont(String postScriptName, File file)
        {
            try
            {
                ...
                {
                    LOG.log(Level.FINER,"Loaded " + postScriptName + " from " + file);
                }
                return ttf;
            }
            catch (IOException e)
            {
                LOG.log(Level.WARNING,"Could not load font file: " + file, e);
            }
            return null;
        }
    ...
    FileSystemFontProvider(FontCache cache)
    {
        ...
        try
        {
            ...
            {
                LOG.log(Level.FINEST,"Will search the local system for fonts");
            }
            ...
            if (LOG.isLoggable(Level.FINEST))
            {
                LOG.log(Level.FINEST,"Found " + files.size() + " fonts on the local system");
            }
            ...
            if (!files.isEmpty())
            {
                ...
                {
                    LOG.log(Level.WARNING,"Building on-disk font cache, this may take a while");
                    scanFonts(files);
                    saveDiskCache();
                    LOG.log(Level.WARNING,"Finished building on-disk font cache, found " + fontInfoList.size()
                            + " fonts");
                }
            }
        }
        ...
        {
            LOG.log(Level.SEVERE,"Error accessing the file system", e);
        }
    }
    ...
    private void scanFonts(List<File> files)
    {
        ...
        for (File file : files)
        {
            ...
            {
                LOG.log(Level.WARNING,"Error parsing font " + file.getPath(), e);
            }
        }
    }
    ...
    private void saveDiskCache()
    {
        try
        {
            ...
            {
                LOG.log(Level.WARNING,"Could not write to font cache", e);
                LOG.log(Level.WARNING,"Installed fonts information will have to be reloaded for each start");
                LOG.log(Level.WARNING,"You can assign a directory to the 'pdfbox.fontcache' property");
            }
        }
        catch (SecurityException e)
        {
            LOG.log(Level.FINER,"Couldn't create writer for font cache file", e);
        }
    }
    ...
    private List<FSFontInfo> loadDiskCache(List<File> files)
    {
        ...
        {
            LOG.log(Level.FINER,"Error checking for file existence", e);
        }
        ...
        if (fileExists)
        {
            try (BufferedReader reader = new BufferedReader(new FileReader(file)))
            {
                ...
                while ((line = reader.readLine()) != null)
                {
                    ...
                    if (parts.length < 10)
                    {
                        LOG.log(Level.WARNING,"Incorrect line '" + line + "' in font disk cache is skipped");
                        continue;
                    }
                    ...
                    {
                        LOG.log(Level.FINER,"Font file " + fontFile.getAbsolutePath() + " not found, skipped");
                    }
            ...
            {
                LOG.log(Level.WARNING,"Error loading font cache, will be re-built", e);
                return null;
            }
        }
        ...
        if (!pending.isEmpty())
        {
            ...
            LOG.log(Level.WARNING,"New fonts found, font cache will be re-built");
            return null;
        }
    ...
    private void addTrueTypeCollection(final File ttcFile) throws IOException
    {
        ...
        {
            LOG.log(Level.WARNING,"Could not load font file: " + ttcFile, e);
        }
    }
    ...
    private void addTrueTypeFont(File ttfFile) throws IOException
    {
        ...
        {
            LOG.log(Level.WARNING,"Could not load font file: " + ttfFile, e);
        }
    }
    ...
    private void addTrueTypeFontImpl(TrueTypeFont ttf, File file) throws IOException
    {
        try
        {
            ...
            if (ttf.getName() != null && ttf.getName().contains("|"))
            {
                ...
                LOG.log(Level.WARNING,"Skipping font with '|' in name " + ttf.getName() + " in file " + file);
            }
            else if (ttf.getName() != null)
            {
                ...
                if (LOG.isLoggable(Level.FINEST))
                {
                    ...
                    if (name != null)
                    {
                        LOG.log(Level.FINEST,format +": '" + name.getPostScriptName() + "' / '" +
                                  name.getFontFamily() + "' / '" +
            ...
            else
            {
                ...
                LOG.log(Level.WARNING,"Missing 'name' entry for PostScript name in font " + file);
            }
        }
        catch (IOException e)
        {
    ...
    private void addType1Font(File pfbFile) throws IOException
    {
        try (InputStream input = new FileInputStream(pfbFile))
        {
            ...
            if (type1.getName() == null)
            {
                ...
                LOG.log(Level.WARNING,"Missing 'name' entry for PostScript name in font " + pfbFile);
                return;
            }
            if (type1.getName().contains("|"))
            {
                fontInfoList.add(new FSIgnored(pfbFile, FontFormat.PFB, "*skippipeinname*"));
                LOG.log(Level.WARNING,"Skipping font with '|' in name " + type1.getName() + " in file " + pfbFile);
                return;
            }
            ...
            if (LOG.isLoggable(Level.FINEST))
            {
                LOG.log(Level.FINEST,"PFB: '" + type1.getName() + "' / '" + type1.getFamilyName() + "' / '" +
                        type1.getWeight() + "'");
            }
        }
        catch (IOException e)
        {
            LOG.log(Level.WARNING,"Could not load font file: " + pfbFile, e);
        }
    }
