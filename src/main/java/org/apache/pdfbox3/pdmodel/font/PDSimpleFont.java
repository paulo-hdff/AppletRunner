...
import org.apache.pdfbox3.pdmodel.font.encoding.DictionaryEncoding;
...
{
    private static final Logger LOG = Logger.getLogger(PDSimpleFont.class.getName());
    ...
    protected void readEncoding() throws IOException
    {
        ...
        if (encodingBase instanceof COSName)
        {
            ...
            {
                LOG.log(Level.WARNING,"Unknown encoding: " + encodingName.getName());
    ...
    public String toUnicode(int code, GlyphList customGlyphList)
    {
        ...
        if (LOG.isLoggable(Level.WARNING) && !noUnicode.contains(code))
        {
            // we keep track of which warnings have been issued, so we don't log multiple times
            noUnicode.add(code);
            if (name != null)
            {
                LOG.log(Level.WARNING,"No Unicode mapping for " + name + " (" + code + ") in font " +
                        getName());
            }
            else
            {
                LOG.log(Level.WARNING,"No Unicode mapping for character code " + code + " in font " +
                        getName());
            }
        }
