...
{
    private static final Logger LOG = Logger.getLogger(PDPageTree.class.getName());
    ...
    private List<COSDictionary> getKids(COSDictionary node)
    {
        ...
        for (int i = 0, size = kids.size(); i < size; i++)
        {
            ...
            {
                LOG.log(Level.WARNING,"COSDictionary expected, but got " +
    ...
    private final class PageIterator implements Iterator<PDPage>
    {
        ...
        private void enqueueKids(COSDictionary node)
        {
            if (isPageTreeNode(node))
            {
                ...
                for (COSDictionary kid : kids)
                {
                    if (set.contains(kid))
                    {
                        ...
                        LOG.log(Level.SEVERE,"This page tree node has already been visited");
                        continue;
                    }
            ...
            else
            {
                ...
                {
                    LOG.log(Level.SEVERE,"Page skipped due to an invalid or missing type "
