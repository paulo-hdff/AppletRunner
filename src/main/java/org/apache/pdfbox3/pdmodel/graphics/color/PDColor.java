...
{
    private static final Logger LOG = Logger.getLogger(PDColor.class.getName());
    ...
    public PDColor(COSArray array, PDColorSpace colorSpace)
    {
        if (array.size() > 0 && array.get(array.size() - 1) instanceof COSName)
        {
            ...
            {
                LOG.log(Level.WARNING,"pattern name in " + array + " isn't a name, ignored");
    ...
    private void initComponents(COSArray array)
    {
        for (int i = 0; i < components.length; i++)
        {
            ...
            {
                LOG.log(Level.WARNING,"color component " + i + " in " + array + " isn't a number, ignored");
            }
        }
    }
