...
{
    private static final Logger LOG = Logger.getLogger(SampledImageReader.class.getName());
    ...
    public static BufferedImage getStencilImage(PDImage pdImage, Paint paint) throws IOException
    {
        ...
        try (InputStream iis = pdImage.createInputStream())
        {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                if (readLen != rowLen)
                {
                    LOG.log(Level.WARNING,"premature EOF, image will be incomplete");
                    break;
                }
            }            
        }
                                          ...
                                          final int width, final int height) throws IOException
    {
        ...
        try (InputStream input = pdImage.createInputStream(options))
        {
            ...
            if (startx == 0 && starty == 0 && scanWidth == width && scanHeight == height && currentSubsampling == 1)
            {
                ...
                {
                    LOG.log(Level.FINER,"Tried reading " + (long) width * height * numComponents + " bytes but only " + inputResult + " bytes read");
                }
            ...
            for (int y = 0; y < starty + scanHeight; ++y)
            {
                ...
                {
                    LOG.log(Level.FINER,"Tried reading " + tempBytes.length + " bytes but only " + inputResult + " bytes read");
                }
            ...
            throws IOException
    {
        ...
        try (ImageInputStream iis = new MemoryCacheImageInputStream(pdImage.createInputStream(options)))
        {
            ...
            if (colorKey != null)
            {
                ...
                {
                    LOG.log(Level.WARNING,"colorKey mask size is " + colorKey.size() +
        ...
        if (cosDecode != null)
        {
            ...
            if (cosDecode.size() != numberOfComponents * 2)
            {
                        ...
                        && cosDecode.get(1) instanceof COSNumber)
                {
                    ...
                    {
                        LOG.log(Level.WARNING,"decode array " + cosDecode
                ...
                LOG.log(Level.SEVERE,"decode array " + cosDecode
