 ...
 */
package org.apache.pdfbox3.pdmodel.graphics.image;
...
import org.apache.pdfbox3.cos.COSInteger;
import org.apache.pdfbox3.cos.COSName;
import org.apache.pdfbox3.cos.COSStream;
import org.apache.pdfbox3.filter.Filter;
import org.apache.pdfbox3.filter.FilterFactory;
import org.apache.pdfbox3.pdmodel.PDDocument;
import org.apache.pdfbox3.pdmodel.common.PDStream;
import org.apache.pdfbox3.pdmodel.graphics.color.PDColorSpace;
import org.apache.pdfbox3.pdmodel.graphics.color.PDDeviceGray;
import org.apache.pdfbox3.pdmodel.graphics.color.PDDeviceRGB;
import org.apache.pdfbox3.pdmodel.graphics.color.PDICCBased;
import org.apache.pdfbox3.pdmodel.graphics.color.PDIndexed;
...
{
    private static final Logger LOG = Logger.getLogger(PNGConverter.class.getName());
            ...
            throws IOException
    {
        ...
        {
            LOG.log(Level.SEVERE,String.format("Invalid bit depth %d.", bitDepth));
            return null;
        }
        if (width <= 0 || height <= 0)
        {
            LOG.log(Level.SEVERE,String.format("Invalid image size %d x %d", width, height));
            return null;
        }
        if (compressionMethod != 0)
        {
            LOG.log(Level.SEVERE,String.format("Unknown PNG compression method %d.", compressionMethod));
            return null;
        }
        if (filterMethod != 0)
        {
            LOG.log(Level.SEVERE,String.format("Unknown PNG filtering method %d.", compressionMethod));
            return null;
        }
        if (interlaceMethod != 0)
        {
            LOG.log(Level.FINER,String.format("Can't handle interlace method %d.", interlaceMethod));
            return null;
        }
        ...
        case 0:
            // Grayscale
            LOG.log(Level.FINER,"Can't handle grayscale yet.");
            return null;
        case 2:
            // Truecolor
            if (state.tRNS != null)
            {
                LOG.log(Level.FINER,"Can't handle images with transparent colors.");
                return null;
            }
        ...
        case 6:
            ...
            return null;
        default:
            LOG.log(Level.SEVERE,"Unknown PNG color type " + colorType);
            return null;
        }
    }
            ...
            throws IOException
    {
        ...
        if (plte == null)
        {
            LOG.log(Level.SEVERE,"Indexed image without PLTE chunk.");
            return null;
        }
        if (plte.length % 3 != 0)
        {
            LOG.log(Level.SEVERE,"PLTE table corrupted, last (r,g,b) tuple is not complete.");
            return null;
        }
        if (state.bitsPerComponent > 8)
        {
            LOG.log(Level.FINER,String.format("Can only convert indexed images with bit depth <= 8, not %d.",
        ...
        if (highVal > 255)
        {
            LOG.log(Level.SEVERE,String.format("Too much colors in PLTE, only 256 allowed, found %d colors.",
                    highVal + 1));
            return null;
        }
            ...
            throws IOException
    {
        ...
        if (state.gAMA != null && !hasICCColorProfile)
        {
            ...
            {
                LOG.log(Level.SEVERE,"Invalid gAMA chunk length " + state.gAMA.length);
                return null;
            }
            ...
            {
                LOG.log(Level.FINER,String.format("We can't handle gamma of %f yet.", gamma));
                return null;
            }
        }
        ...
        if (state.cHRM != null && !hasICCColorProfile)
        {
            ...
            {
                LOG.log(Level.SEVERE,"Invalid cHRM chunk length " + state.cHRM.length);
                return null;
            }
            LOG.log(Level.FINER,"We can not handle cHRM chunks yet.");
            return null;
        }
        ...
        (PDDocument document, PDColorSpace colorSpace, PNGConverterState state) throws IOException
    {
        ...
        if (state.iCCP != null)
        {
            ...
            {
                LOG.log(Level.SEVERE,"Invalid iCCP chunk, to few bytes");
                return null;
            }
            byte compressionMethod = state.iCCP.bytes[state.iCCP.start + iccProfileDataStart];
            if (compressionMethod != 0)
            {
                LOG.log(Level.SEVERE,String.format("iCCP chunk: invalid compression method %d",
                        compressionMethod));
                return null;
            }
    ...
    static boolean checkConverterState(PNGConverterState state)
    {
        ...
        {
            LOG.log(Level.SEVERE,"Invalid IHDR chunk.");
            return false;
        }
        if (!checkChunkSane(state.PLTE))
        {
            LOG.log(Level.SEVERE,"Invalid PLTE chunk.");
            return false;
        }
        if (!checkChunkSane(state.iCCP))
        {
            LOG.log(Level.SEVERE,"Invalid iCCP chunk.");
            return false;
        }
        if (!checkChunkSane(state.tRNS))
        {
            LOG.log(Level.SEVERE,"Invalid tRNS chunk.");
            return false;
        }
        if (!checkChunkSane(state.sRGB))
        {
            LOG.log(Level.SEVERE,"Invalid sRGB chunk.");
            return false;
        }
        if (!checkChunkSane(state.cHRM))
        {
            LOG.log(Level.SEVERE,"Invalid cHRM chunk.");
            return false;
        }
        if (!checkChunkSane(state.gAMA))
        {
            LOG.log(Level.SEVERE,"Invalid gAMA chunk.");
            return false;
        }
        ...
        {
            LOG.log(Level.SEVERE,"No IDAT chunks.");
            return false;
        }
        for (Chunk idat : state.IDATs)
        {
            ...
            {
                LOG.log(Level.SEVERE,"Invalid IDAT chunk.");
                return false;
            }
        }
        return true;
    ...
    static boolean checkChunkSane(Chunk chunk)
    {
        ...
        if (ourCRC != chunk.crc)
        {
            LOG.log(Level.SEVERE,String.format("Invalid CRC %08X on chunk %08X, expected %08X.", ourCRC,
    ...
    private static PNGConverterState parsePNGChunks(byte[] imageData)
    {
        ...
        {
            LOG.log(Level.SEVERE,"ByteArray way to small: " + imageData.length);
            return null;
        }
        ...
        {
            LOG.log(Level.SEVERE,String.format("First Chunktype was %08X, not IHDR", firstChunkType));
            return null;
        }
        ...
        while (ptr + 12 <= imageData.length)
        {
            ...
            {
                LOG.log(Level.SEVERE,"Not enough bytes. At offset " + ptr + " are " + chunkLength
            ...
            case CHUNK_IHDR:
                if (state.IHDR != null)
                {
                    LOG.log(Level.SEVERE,"Two IHDR chunks? There is something wrong.");
                    return null;
                }
                state.IHDR = chunk;
                break;
            ...
            case CHUNK_PLTE:
                ...
                if (state.PLTE != null)
                {
                    LOG.log(Level.SEVERE,"Two PLTE chunks? There is something wrong.");
                    return null;
                }
                state.PLTE = chunk;
                break;
            ...
            case CHUNK_TRNS:
                ...
                if (state.tRNS != null)
                {
                    LOG.log(Level.SEVERE,"Two tRNS chunks? There is something wrong.");
                    return null;
                }
                state.tRNS = chunk;
                break;
            ...
            case CHUNK_ICCP:
                ...
                break;
            case CHUNK_SBIT:
                LOG.log(Level.FINER,"Can't convert PNGs with sBIT chunk.");
                break;
            case CHUNK_SRGB:
            ...
            case CHUNK_TIME:
                ...
                break;
            default:
                LOG.log(Level.FINER,String.format("Unknown chunk type %08X, skipping.", chunkType));
                break;
            }
            ptr += chunkLength;
        ...
        LOG.log(Level.SEVERE,"No IEND chunk found.");
        return null;
    }
