 ...
 */
package org.apache.pdfbox3.pdmodel.graphics.blend;
...
public final class BlendComposite implements Composite
{
    ...
    public static Composite getInstance(BlendMode blendMode, float constantAlpha)
    {
        if (constantAlpha < 0)
        {
            LOG.log(Level.WARNING,"using 0 instead of incorrect Alpha " + constantAlpha);
            constantAlpha = 0;
        }
        else if (constantAlpha > 1)
        {
            LOG.log(Level.WARNING,"using 1 instead of incorrect Alpha " + constantAlpha);
            constantAlpha = 1;
        }
        if (blendMode == null)
        {
