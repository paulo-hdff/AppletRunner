 ...
 */
package org.apache.pdfbox3.pdfparser;
...
import org.apache.pdfbox3.cos.COSArray;
import org.apache.pdfbox3.cos.COSBase;
import org.apache.pdfbox3.cos.COSDictionary;
import org.apache.pdfbox3.cos.COSName;
import org.apache.pdfbox3.cos.COSNull;
import org.apache.pdfbox3.cos.COSNumber;
import org.apache.pdfbox3.cos.COSObject;
import org.apache.pdfbox3.cos.COSObjectKey;
import org.apache.pdfbox3.cos.COSStream;
import org.apache.pdfbox3.cos.ICOSParser;
import org.apache.pdfbox3.io.IOUtils;
import org.apache.pdfbox3.io.RandomAccessRead;
import org.apache.pdfbox3.io.RandomAccessReadView;
import org.apache.pdfbox3.pdfparser.XrefTrailerResolver.XRefType;
import org.apache.pdfbox3.pdmodel.encryption.AccessPermission;
import org.apache.pdfbox3.pdmodel.encryption.DecryptionMaterial;
import org.apache.pdfbox3.pdmodel.encryption.InvalidPasswordException;
import org.apache.pdfbox3.pdmodel.encryption.PDEncryption;
import org.apache.pdfbox3.pdmodel.encryption.ProtectionPolicy;
import org.apache.pdfbox3.pdmodel.encryption.PublicKeyDecryptionMaterial;
import org.apache.pdfbox3.pdmodel.encryption.SecurityHandler;
import org.apache.pdfbox3.pdmodel.encryption.StandardDecryptionMaterial;
...
public class COSParser extends BaseParser implements ICOSParser
{
    ...
    public static final String SYSPROP_EOFLOOKUPRANGE =
            "org.apache.pdfbox3.pdfparser.nonSequentialPDFParser.eofLookupRange";
    ...
    private static final Logger LOG = Logger.getLogger(COSParser.class.getName());
        ...
        while (prev > 0)
        {
            ...
            if (source.peek() == X)
            {
                ...
                if(trailer.containsKey(COSName.XREF_STM))
                {
                    ...
                    {
                        LOG.log(Level.WARNING,"/XRefStm offset " + streamOffset + " is incorrect, corrected to " + fixedOffset);
                    ...
                    if (streamOffset > 0)
                    {
                        ...
                        catch (IOException ex)
                        {
                            if (isLenient)
                            {
                                LOG.log(Level.SEVERE,"Failed to parse /XRefStm at offset " + streamOffset, ex);
                            }
                            else
                            {
                                throw ex;
                    ...
                    else
                    {
                        if(isLenient)
                        {
                            LOG.log(Level.SEVERE,"Skipped XRef stream due to a corrupt offset:"+streamOffset);
                        }
                        else
                        {
    ...
    private long getStartxrefOffset() throws IOException
    {
        ...
        if (bufOff < 0)
        {
            if (isLenient) 
            {
                ...
                bufOff = buf.length;
                LOG.log(Level.FINER,"Missing end of file marker '" + new String(EOF_MARKER) + "'");
            } 
            else 
            {
            ...
            throws IOException
    {
        ...
        if (offsetOrObjstmObNr == null && isLenient)
        {
            ...
            if (offsetOrObjstmObNr != null)
            {
                LOG.log(Level.FINER,"Set missing offset " + offsetOrObjstmObNr + " for object " + objKey);
            ...
            throws IOException
    {
        ...
        if (!endObjectKey.startsWith(ENDOBJ_STRING))
        {
            if (isLenient)
            {
                LOG.log(Level.WARNING,"Object (" + readObjNr + ":" + readObjGen + ") at offset "
    ...
    protected COSBase parseObjectStreamObject(long objstmObjNr, COSObjectKey key) throws IOException
    {
        ...
        if (objstmBaseObj instanceof COSStream)
        {
            ...
            catch (IOException ex)
            {
                if (isLenient)
                {
                    LOG.log(Level.SEVERE,"object stream " + objstmObjNr
    ...
    private COSNumber getLength(final COSBase lengthBaseObj) throws IOException
    {
        ...
        if (lengthBaseObj instanceof COSObject)
        {
            ...
            {
                LOG.log(Level.WARNING,"Length object (" + lengthObj.getObjectNumber() + " "
    ...
    protected COSStream parseCOSStream(COSDictionary dic) throws IOException
    {
        ...
        if (streamLengthObj == null)
        {
            if (isLenient)
            {
               LOG.log(Level.WARNING,"The stream doesn't provide any stream length, using fallback readUntilEnd, at offset "
                    + source.getPosition());
            }
            else
            {
        ...
        {
            LOG.log(Level.WARNING,"stream ends with 'endobj' instead of 'endstream' at offset "
                    + source.getPosition());
        ...
        {
            LOG.log(Level.WARNING,"stream ends with '" + endStream + "' instead of 'endstream' at offset "
                    + source.getPosition());
    ...
    private boolean validateStreamLength(long streamLength) throws IOException
    {
        ...
        if (expectedEndOfStream > fileLen)
        {
            ...
            LOG.log(Level.WARNING,"The end of the stream is out of range, using workaround to read the stream, "
        ...
        else
        {
            ...
            {
                streamLengthIsValid = false;
                LOG.log(Level.WARNING,"The end of the stream doesn't point to the correct offset, using workaround to read the stream, "
                        + "stream start position: " + originOffset + ", length: " + streamLength
                        + ", expected end position: " + expectedEndOfStream);
            }
        ...
        if (isWhitespace(nextValue))
        {
            ...
            if (isDigit())
            {
                ...
                catch (IOException exception)
                {
                    ...
                    LOG.log(Level.FINER,"No Xref stream at given location " + startXRefOffset, exception);
                    source.seek(startXRefOffset);
                }
            }
        }
    ...
    private long calculateXRefFixedOffset(long objectOffset) throws IOException
    {
        if (objectOffset < 0)
        {
            LOG.log(Level.SEVERE,"Invalid object offset " + objectOffset + " when searching for a xref table/stream");
            return 0;
        }
        ...
        if (newOffset > -1)
        {
            LOG.log(Level.FINER,"Fixed reference for xref table/stream " + objectOffset + " -> " + newOffset);
            return newOffset;
        }
        LOG.log(Level.SEVERE,"Can't find the object xref table/stream at offset " + objectOffset);
        return 0;
    }
    ...
    private boolean validateXrefOffsets(Map<COSObjectKey, Long> xrefOffset) throws IOException
    {
        ...
        for (Entry<COSObjectKey, Long> objectEntry : xrefOffset.entrySet())
        {
            ...
            if (objectOffset != null && objectOffset >= 0)
            {
                ...
                {
                    LOG.log(Level.FINER,"Stop checking xref offsets as at least one (" + objectKey
    ...
    private void checkXrefOffsets() throws IOException
    {
        ...
        if (!validateXrefOffsets(xrefOffset))
        {
            ...
            {
                LOG.log(Level.FINER,"Replaced read xref table with the results of a brute force search");
                xrefOffset.clear();
            ...
            Map<COSObjectKey, Long> xrefOffset) throws IOException
    {
        ...
        try 
        {
            ...
            if (source.getPosition() == offset)
            {
                ...
                if (source.getPosition() < offset)
	            {
	                ...
	                else
	                {
	                    ...
	                    {
	                        LOG.log(Level.FINER,"Found the object " + newObjKey + " instead of " //
            ...
            {
                LOG.log(Level.WARNING,"found wrong object number. expected [" + objectKey.getNumber() + "] found [" + foundObjectNumber + "]");
                if (!isLenient)
                {
                    return null;
                }
        ...
        catch (IOException exception)
        {
            ...
            LOG.log(Level.FINER,"No valid object at given location " + offset + " - ignoring", exception);
        }
        return null;
    }
    ...
    private int checkPagesDictionary(COSDictionary pagesDict, Set<COSObject> set)
    {
        ...
        if (kidsArray != null)
        {
            ...
            for (COSBase kid : kidsList)
            {
                ...
                {
                    LOG.log(Level.WARNING,"Removed null object " + kid + " from pages dictionary");
                    kidsArray.remove(kid);
                }
        ...
        if (isLenient)
        {
            ...
            while (nextCharacter != 't' && isDigit(nextCharacter))
            {
                if (source.getPosition() == trailerOffset)
                {
                    ...
                    LOG.log(Level.WARNING,"Expected trailer object at offset " + trailerOffset
                            + ", keep trying");
                }
                readLine();
    ...
    private boolean parseHeader(String headerMarker, String defaultVersion) throws IOException
    {
        ...
        if (header.startsWith(headerMarker) && !header.matches(headerMarker + "\\d.\\d"))
        {
            if (header.length() < headerMarker.length() + 3)
            {
                ...
                LOG.log(Level.FINER,"No version found, set to " + defaultVersion + " as default.");
            }
            else
            {
        ...
        {
            LOG.log(Level.FINER,"Can't parse the header version.", exception);
        }
        if (headerVersion < 0)
        {
            if (isLenient)
    ...
    protected boolean parseXrefTable(long startByteOffset) throws IOException
    {
        ...
        {
            LOG.log(Level.WARNING,"skipping empty xref table");
            return false;
        }
        ...
        while(true)
        {
            ...
            {
                LOG.log(Level.WARNING,"Unexpected XRefTable Entry: " + currentLine);
                return false;
            }
            // first obj id
            long currObjID;
            ...
            {
                LOG.log(Level.WARNING,"XRefTable: invalid ID for the first object: " + currentLine);
                return false;
            }
            ...
            {
                LOG.log(Level.WARNING,"XRefTable: invalid number of objects: " + currentLine);
                return false;
            }
            ...
            for(int i = 0; i < count; i++)
            {
                ...
                {
                    LOG.log(Level.WARNING,"invalid xref line: " + currentLine);
                    break;
                }
