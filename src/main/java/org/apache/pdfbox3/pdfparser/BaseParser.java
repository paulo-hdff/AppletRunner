...
public abstract class BaseParser
{
     ...
     * Log instance.
     */
    private static final Logger LOG = Logger.getLogger(BaseParser.class.getName());
    ...
    private COSBase parseCOSDictionaryValue() throws IOException
    {
        ...
        {
            LOG.log(Level.SEVERE,"expected number, actual=" + value + " at offset " + numOffset);
            return COSNull.NULL;
        }
        if (!(generationNumber instanceof COSInteger))
        {
            LOG.log(Level.SEVERE,"expected number, actual=" + generationNumber + " at offset " + genOffset);
            return COSNull.NULL;
        }
        long objNumber = ((COSInteger) value).longValue();
        if (objNumber <= 0)
        {
            LOG.log(Level.WARNING,"invalid object number value =" + objNumber + " at offset " + numOffset);
            return COSNull.NULL;
        }
        int genNumber = ((COSInteger) generationNumber).intValue();
        if (genNumber < 0)
        {
            LOG.log(Level.SEVERE,"invalid generation number value =" + genNumber + " at offset " + numOffset);
            return COSNull.NULL;
        }
    ...
    protected COSDictionary parseCOSDictionary(boolean isDirect) throws IOException
    {
        ...
        while (true)
        {
            ...
            else
            {
                ...
                LOG.log(Level.WARNING,"Invalid dictionary, found: '" + c + "' but expected: '/' at offset "
                        + source.getPosition());
        ...
        {
            LOG.log(Level.WARNING,"Invalid dictionary, can't find end of dictionary at offset "
                    + source.getPosition());
        }
        return obj;
    }
    ...
    private boolean parseCOSDictionaryNameValuePair(COSDictionary obj) throws IOException
    {
        ...
        {
            LOG.log(Level.WARNING,"Empty COSName at offset " + source.getPosition());
        }
        COSBase value = parseCOSDictionaryValue();
        skipSpaces();
        if (value == null)
        {
            LOG.log(Level.WARNING,"Bad dictionary declaration at offset " + source.getPosition());
            return false;
        }
        else if (value instanceof COSInteger && !((COSInteger) value).isValid())
        {
            LOG.log(Level.WARNING,"Skipped out of range number value at offset " + source.getPosition());
        }
        else
        {
    ...
    protected COSArray parseCOSArray() throws IOException
    {
        ...
        while (((i = source.peek()) > 0) && ((char) i != ']'))
        {
            ...
            if( pbo instanceof COSObject )
            {
                ...
                if (po.size() > 1 && po.get(po.size() - 1) instanceof COSInteger)
                {
                    ...
                    if (po.size() > 0 && po.get(po.size() - 1) instanceof COSInteger)
                    {
                        ...
                        {
                            LOG.log(Level.WARNING,"Invalid value(s) for an object key " + number.longValue()
                                    + " " + genNumber.intValue());
                        }
                    }
                }
            ...
            if (pbo == null)
            {
                ...
                LOG.log(Level.WARNING,"Corrupt array element at offset " + source.getPosition()
        ...
        while (c != -1)
        {
            ...
            if (ch == '#')
            {
                ...
                else
                {
                    ...
                    if (ch2 == -1 || ch1 == -1)
                    {
                        LOG.log(Level.SEVERE,"Premature EOF in BaseParser#parseCOSName");
                        c = -1;
                        break;
                    }
                    source.rewind(1);
    ...
    private boolean isValidUTF8(byte[] input)
    {
        ...
        {
            LOG.log(Level.FINER,"Character could not be decoded using StandardCharsets.UTF_8 - returning false", e);
            return false;
        }
    }
    ...
    protected COSBase parseDirObject() throws IOException
    {
        ...
        default:
            ...
            {
                LOG.log(Level.WARNING,"Skipped unexpected dir object = '" + badString + "' at offset "
