...
public class PDFStreamParser extends BaseParser
{
     ...
     * Log instance.
     */
    private static final Logger LOG = Logger.getLogger(PDFStreamParser.class.getName());
        ...
        switch (c)
        {
            case '<':
                ...
                if (c == '<')
                {
                    ...
                    {
                        LOG.log(Level.WARNING,"Stop reading invalid dictionary from content stream at offset "
                                + source.getPosition());
                        close();
                        return null;
                    }
            ...
            case '[':
                ...
                {
                    LOG.log(Level.WARNING,"Stop reading invalid array from content stream at offset "
                            + source.getPosition());
                    close();
                    return null;
                }
            ...
            case 'B':
                ...
                if (nextOperator.equals(OperatorName.BEGIN_INLINE_IMAGE))
                {
                    ...
                    while( (nextToken = parseNextToken()) instanceof COSName )
                    {
                        ...
                        {
                            LOG.log(Level.WARNING,"Unexpected token in inline image dictionary at offset " +
                    ...
                    if (nextToken instanceof Operator)
                    {
                        ...
                        {
                            LOG.log(Level.WARNING,"empty inline image at stream offset " + source.getPosition());
                        }
    ...
    private boolean hasNoFollowingBinData() throws IOException
    {
        ...
        if (!noBinData)
        {
            LOG.log(Level.WARNING,"ignoring 'EI' assumed to be in the middle of inline image at stream offset " + 
                    source.getPosition());
        }
        return noBinData;
    }
